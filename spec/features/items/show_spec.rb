...
RSpec.describe 'item show page', type: :feature do
  ...
  it 'can edit review for item and post flash message when review is edited' do
    visit "/items/#{@chain.id}"

    within "#review-#{@average_review.id}" do
      click_link "edit review"
    end

    expect(current_path).to eq("/items/#{@chain.id}/#{@average_review.id}/edit")

    title_2 = "This a better than average product"
    content_2 = "This product was just above average"
    rating_2 = 4

    fill_in :title, with: title_2
    fill_in :content, with: content_2
    fill_in :rating, with: rating_2
    ...
    within "#review-#{@average_review.id}" do
      ...
      expect(page).to have_content("Rating: #{rating_2}")
    end
  end

  it 'shows alert flash messages when new review form is not completely filled' do
    visit "/items/#{@chain.id}"
    click_link "add new review"
  ...
  it 'shows alert flash message when edit review form is not completely filled' do
    visit "/items/#{@chain.id}"
